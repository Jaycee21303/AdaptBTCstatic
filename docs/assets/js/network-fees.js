...
const loadFees = async () => {
  ...
  try {
    ...
    if (!feesRes.ok) throw new Error('Fee request failed');
    ...
    latestPayload = {
      fees: {
        fastest: feesJson.fastestFee,
        half_hour: feesJson.halfHourFee,
        hour: feesJson.hourFee,
        minimum: feesJson.minimumFee,
      },
      assumptions: { vbytes: 140 },
      btc_usd: btcUsd,
      timestamp: Math.floor(Date.now() / 1000),
    };

    renderRates(latestPayload);
    renderEstimate();
  } catch (error) {
    latestPayload = null;
