(function () {
  ...
  async function fetchHistoryRange(rangeDays = 'max') {
    const historyUrl = `${API_BASE}/coins/bitcoin/market_chart?vs_currency=usd&days=${rangeDays}&interval=daily`;

    const data = await fetchJsonWithRetry(historyUrl);
  ...
  async function fetchBtcSnapshot() {
    ...
    const snapshotUrl = `${API_BASE}/coins/bitcoin?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`;

    snapshotPromise = fetchJsonWithRetry(snapshotUrl)
      .then((data) => {
      ...
      .catch((error) => {
        console.warn('BTC snapshot failed', error);
