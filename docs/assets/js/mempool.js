...
const loadSummary = async () => {
  ...
  try {
    ...
    const data = await response.json();
    const totals = computeBuckets([]);
    const timestamp = Date.now() / 1000;
    updateMempoolChart(totals, Date.now());
    updateChips(
      { ...data, as_of: timestamp, timestamp, totalFees: data.total_fee, vbytes: data.vsize, stale: false },
      recommendedFees,
    );
    chartStatusEl.textContent = `Live mempool depth · Updated ${formatUpdateTime(timestamp)}`;
    chartStatusEl.classList.remove('warning');
    chartStatusEl.classList.add('positive');
  } catch (error) {
    if (chartStatusEl) {
...
const loadFeed = async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error('request failed');
    const data = await response.json();
    await renderFeed(Array.isArray(data) ? data : data.transactions || []);
    const timestamp = Date.now() / 1000;
    feedStatusEl.textContent = `Live unconfirmed flow · ${formatUpdateTime(timestamp)}`;
    feedStatusEl.classList.remove('warning');
    feedStatusEl.classList.add('positive');
  } catch (error) {
...
const loadGuidance = async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error('request failed');
    const data = await response.json();
    recommendedFees = { ...data, timestamp: Date.now() / 1000, stale: false };
    renderGuidance(recommendedFees);
    updateChips(null, recommendedFees);
  } catch (error) {
...
const loadBlocks = async () => {
  ...
  try {
    ...
    const timestamp = Date.now() / 1000;
    blockStatusEl.textContent = `Projected backlog · ${formatUpdateTime(timestamp)}`;
    blockStatusEl.classList.remove('warning');
    blockStatusEl.classList.add('positive');
  } catch (error) {
...
const initPreview = async () => {
  ...
  try {
    ...
    const enrichedSummary = summary
      ? { ...summary, as_of: Date.now() / 1000, totalFees: summary.total_fee, vbytes: summary.vsize, stale: false }
      : null;
    renderPreview(enrichedSummary, rec);
  } catch (error) {
    const statusEl = previewCard.querySelector('[data-preview-status]');
    if (statusEl) {
